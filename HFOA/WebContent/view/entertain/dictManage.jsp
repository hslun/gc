...
document.oncontextmenu=function(e){return false;};
	...
	function getTree(){
		$('#t1').tree({
			...
			url:'${pageContext.request.contextPath}/getByNodeType',
...
// 			mark:node.attributes.url
	...
	function saveType(){
		...
		if(flag=='add'){
			...
			$.ajax({
				type:'post',
				url:'${pageContext.request.contextPath}/saveDict',
				cache:false,
				data:{
					id:node.id,
			...
			$.ajax({
				type:'post',
				url:'${pageContext.request.contextPath}/updateDict',
				cache:false,
				data:{
	...
	function removetree(){
		...
		if(node){
			...
			$.messager.confirm('确认对话框','你确定删除这条记录吗？',function(r){
			       if (r){
						$.post('${pageContext.request.contextPath}/deleteDict',{id:node.id},function(result){
							//给出提示信息
							if(result=='1'){
