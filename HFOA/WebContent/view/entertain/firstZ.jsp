...
// 				getTree();
      ...
      $("#entertain").tree({
        checkbox: false,
        url: "${pageContext.request.contextPath}/Menu/entertainL.json",
        onClick: function (node) {
          var content = node.text;
          addPanel(node.url, content);
        }
      ...
      $("#budget").tree({
        checkbox: false,
        url: "${pageContext.request.contextPath}/Menu/budget.json",
        onClick: function (node) {
          var content = node.text;
          addPanel(node.url, content);
        }
      ...
      $("#privateCar").tree({
        checkbox: false,
        url: "${pageContext.request.contextPath}/Menu/privateCar.json",
        onClick: function (node) {
          var content = node.text;
          addPanel(node.url, content);
        }
      ...
      $("#commonCar").tree({
        checkbox: false,
        url: "${pageContext.request.contextPath}/Menu/commonCar.json",
        onClick: function (node) {
          var content = node.text;
          addPanel(node.url, content);
        }
      ...
      $("#officialSeal").tree({
        checkbox: false,
        url: "${pageContext.request.contextPath}/Menu/officialSeal.json",
        onClick: function (node) {
          var content = node.text;
          addPanel(node.url, content);
        }
      ...
      $("#travelExpenses").tree({
        checkbox: false,
        url: "${pageContext.request.contextPath}/Menu/travelExpenses.json",
        onClick: function (node) {
          var content = node.text;
          addPanel(node.url, content);
        }
      ...
      $("#operationLog").tree({
        checkbox: false,
        url: "${pageContext.request.contextPath}/Menu/operationLog.json",
        onClick: function (node) {
          var content = node.text;
          addPanel(node.url, content);
        }
      ...
      $("#userInfo").tree({
        checkbox: false,
        url: "${pageContext.request.contextPath}/Menu/sysUserManage.json",
        onClick: function (node) {
          var content = node.text;
          addPanel(node.url, content);
        }
...
<div data-options="region:'west',split:true,title:'导航菜单'"
  ...
  <div id="nav" class="easyui-accordion"
    ...
    <%-- <c:forEach items="${sessionScope.titles }" var="title">
      <c:if test="${title.type=='title' }">
        <div title="${title.titleName }" data-options="iconCls:'icon-rz'" style="overflow:auto;">
          <ul id="" class="easyui-tree" style="width: 100%;height:100%;">
            <c:forEach items="${sessionScope.menus }" var="menu">
              <c:if test="${menu.parentId==title.id }">
                <li><a href=javascript:addPanel("${menu.url }","${menu.titleName }")>${menu.titleName }</a>
                </li>
              </c:if>
            </c:forEach>
          </ul>
    ...
    <div title="招待预算信息" data-options="iconCls:'icon-rz'" style="overflow:auto;">
      <ul id="budget" class="easyui-tree" style="width: 100%;height:100%;">
      </ul>
    </div>

    <div title="私车公用信息" data-options="iconCls:'icon-rz'" style="overflow:auto;">
      <ul id="privateCar" class="easyui-tree" style="width: 100%;height:100%;">
      </ul>
    </div>

    <div title="公车使用信息" data-options="iconCls:'icon-rz'" style="overflow:auto;">
      <ul id="commonCar" class="easyui-tree" style="width: 100%;height:100%;">
      </ul>
    </div>

    <div title="公章使用信息" data-options="iconCls:'icon-rz'" style="overflow:auto;">
      <ul id="officialSeal" class="easyui-tree" style="width: 100%;height:100%;">
      </ul>
    </div>

    <div title="差旅费报销" data-options="iconCls:'icon-rz'" style="overflow:auto;">
      <ul id="travelExpenses" class="easyui-tree" style="width: 100%;height:100%;">
      </ul>
    </div>

    <div title="操作日志信息" data-options="iconCls:'icon-rz'" style="overflow:auto;">
      <ul id="operationLog" class="easyui-tree" style="width: 100%;height:100%;">
      </ul>
    </div>
    <div title="用户权限管理" data-options="iconCls:'icon-rz'" style="overflow:auto;">
      <ul id="userInfo" class="easyui-tree" style="width: 100%;height:100%;">
      </ul>
    </div>
