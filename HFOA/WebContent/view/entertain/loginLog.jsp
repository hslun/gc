...
// 			}
	...
	$(function() {
		...
		loadGrid('${pageContext.request.contextPath}/operateRecord/display',null);
		...
		$("#search_ok").click(
						function() {
							var param = {"realName":$("#amanager").textbox("getText").trim(),
										 "startTime":$("#astarttime").textbox("getText"),
										 "endTime":$("#aendtime").textbox("getText")};
// 							$.post(
// 									"${pageContext.request.contextPath}/operateRecord/displayByPage",
// 									param, function(result) {
// 										$("#searchUser").dialog("close");
// 												$("#llbinfo").datagrid("loadData", result);
// 											});
							$("#searchUser").dialog("close");
							loadGrid('${pageContext.request.contextPath}/operateRecord/displayByPage',param);
						});
	...
	function  remove(){
	    ...
	    if (row){
	        $.messager.confirm('确认对话框','你确定删除这条记录吗？',function(r){
	            if (r){
	                $.post('${pageContext.request.contextPath}/operateRecord/deleteRecord',{id:row.id},function(result){
	                    if (result=="1"){
	                        $('#llbinfo').datagrid('reload');    // reload the user data
	                        $.messager.show({
	                            title: 'Message',
...
<body>	<!-- 审核登记对话框 -->
	...
	<div class="mdiv" style="width:100%;height:94%;">
		<table id="llbinfo" class="easyui-datagrid" title="用户登录日志"
			style="width: auto; height:100%;">
			<thead>
				<tr>
				 	<th
						data-options="field:'operateTime',width:fixWidth(0.25),align:'center'">操作时间</th>
					<th
						data-options="field:'realName',width:fixWidth(0.25),align:'center'">操作人</th>
					<th
						data-options="field:'operateDevice',width:fixWidth(0.25),align:'center'">登录位置</th>
					<th 
						data-options="field:'operateInfo',width:fixWidth(0.25),align:'center'">描述</th>
				</tr>
			</thead>
		</table>  
	</div>
