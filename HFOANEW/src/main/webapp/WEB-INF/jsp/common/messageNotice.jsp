...
// 			}
	...
	$(function() {
		...
		$("#btn_ok").click(function(){
		    var html = introEditor.html(); //获取编辑框内容，匹配img.src
			var reg = /<img\b.*?(?:\>|\/>)/gi;
		    var array = html.match(reg);
		    var regSrc = /\bsrc\b\s*=\s*[\'\"]?([^\'\"]*)[\'\"]?/i;
		    var result = new Array();    //编辑框内剩余的图片，需要保存的
		    for(index in array){
		    	//var str = array[index].match(regSrc)[1].split("image/")[1];
		    	var str = array[index].match(regSrc)[1];
		    	result.push(str);
		    }
		    
		    var inputStr = $("#postImg").val().split(",");
		    var merge = result.concat(inputStr).filter(function(v, i, arr) {return arr.indexOf(v) == arr.lastIndexOf(v);});

		    for (x in merge){
		    	var imgUrl = merge[x].split("/images/")[1];
		    	var param = {fileAbsSavePath:imgUrl};
		    	//删除图片
				$.post("${pageContext.request.contextPath}/notice/deleteFile",param, function(result) {
					
				});
		    }
		    //result 转成字符串  放到 hidden 的 input里去
		    $("#postImg").val(result.join(","));
